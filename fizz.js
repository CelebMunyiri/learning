...
function gcdStringApproach2(str1,str2){
  while(1<=str1.length,str2.length<=1000){
  let strr1=str1.toUpperCase();
  let strr2=str2.toUpperCase();
  let divisor="";
  
  for(let j=0;j<strr2.length;j++){
    if(strr1[j]==strr2[j]){
      divisor+=strr1[j];
    }
...
console.log(gcdStringApproach2("ABCAABC","ABCA"));
...
function bonAppetit(bill,k,b){
  ...
  let goodShare = sum/2 - (bill[k]/2);

  if(b==goodShare){
    console.log("Bon Appetit");
  } else{
    console.log(b-goodShare);
  }
